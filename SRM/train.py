...
config['input_size'] = tuple(list(config["patch_shape"])+[5]) if config["patch_shape"] else tuple(list(config["img_shape"])+[5])
config['nb_classes'] = 1
...
config["learning_rate_epochs"] = 20#None
...
config["save_fpath"] = r'..\result_srm\unet3d_1'
if not os.path.exists(config["save_fpath"]):
    os.mkdir(config['save_fpath'])
config["data_fpath"] = r'..\data\SRM'
...
def get_data(data_fpath, split_rate):
    ...
    for i in nii_dirs:
        ...
        if i in val_list:
            ...
            image=np_utils.to_categorical(np.expand_dims(img.get_fdata(), axis=-1)[...,0],5).astype(np.float32)
...
def main():
    ...
    for name in train_name:
        ...
        image=np_utils.to_categorical(np.expand_dims(itk_img.get_fdata(), axis=-1)[...,0],5).astype(np.float32)
        ...
        if config["augment"]:
            if not os.path.exists(os.path.join(r'..\data','SRM' ,'aug')):
                os.mkdir(os.path.join(r'..\data','SRM' , 'aug'))
            augment_times = config["aug_times"]
            aug_train = os.path.join(r'..\data', 'SRM' ,'aug', 'TrainImg')
            if not os.path.exists(aug_train):
                os.mkdir(aug_train)
            aug_mask = os.path.join(r'..\data', 'SRM' ,'aug', 'TrainMask')
            ...
            while augment_times:
                ...
                out_image=np_utils.to_categorical(np.expand_dims(out_img.get_fdata(), axis=-1)[...,0],5).astype(np.float32)
    ...
    if config["model_file"]:
        ...
        model = unet_model_3d(input_shape=config["input_size"], nb_classes=config["nb_classes"],
                              initial_learning_rate=config["initial_lr"], deconvolution=config["deconv"],
                              drop = config["drop"],weight_decay=config["l2_regular"],depth=config["depth"],
                              n_base_filters=config['initial_filter_num'],batch_norm=config["bn"],
                              instance_norm=config["instance_norm"],
                              activation_name=config['activation_fun'])
