# -*- coding: utf-8 -*-
"""
Created on Thu Jul 22 22:51:26 2021
...
for name in names:
    ...
    mask_fdata = sitk.GetArrayFromImage(mask)

    # 裁剪
    (z, x, y) = mask_fdata.shape
    print(x,y,z)
    start, end = caijian(mask_fdata, z)
    print(start, end)
    if start - 2 < 0:
        start = 0
    else:
    ...
    saveimg = img_fdata[start:end, :, :]
    savemask =mask_fdata[start:end, :, :]
    savemask = sitk.GetImageFromArray(savemask)
    ...
    ct_adjust = window_transform(saveimg, 400, 40, normal=False)
    imagelist=[]
    for i in range(ct_adjust.shape[0]):
        img_slice=ct_adjust[i,:,:]
        img_slice_c=clahe.apply(img_slice)
        imagelist.append(img_slice_c)
    saveimg=np.array(imagelist)
